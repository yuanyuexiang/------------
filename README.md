...
每次生成保留独立文件和输入指纹，审查记录绑定具体包及生成版本。正式导出要求人工确认 TRM、输入资料未变、当前规则下无否决项、所有文件待补充清零；`force=true` 会被拒绝。导出清理在副本上执行，旧正式件下载也会重新检查状态。

工作台提供整改入口和具体阻断原因，并恢复最近生成结果、审查清单和正式文件链接。资料修改后重新生成、复查；规则配置修改后重新审查。版本记录使用现有 `projects.results` JSON 字段，无需数据库迁移；旧文件仅可作草稿下载，正式导出前需重新生成。

回归：`pytest -q tests/test_export_workflow.py tests/test_score_export.py tests/test_api_flow.py`。
