...
启动：uvicorn apps.jb_api.main:app --reload --port 8000
"""
...
app.add_middleware(
    CORSMiddleware,
    allow_origins=["http://localhost:5173"],  # jb-web dev server
    allow_methods=["*"],
    allow_headers=["*"],
)
...
async def create_project(file: UploadFile):
    ...
    workdir = tempfile.mkdtemp(prefix="jb_{}_".format(project_id))
    ...
    try:
        ...
        raise HTTPException(422, "解析失败: {}".format(exc))
